...
# =============================================================================
# FIXES APPLIED vs original PDQN_DS.py / PDQN_DS_New.py
# =============================================================================
# [CRITICAL-1] Separate actor_encoder and critic_encoder with their own target
#              networks and optimisers (originally one shared encoder sat in
#              both optimisers causing competing gradient updates).
#              --> Inherited from PDQN_DS_New.py, retained here.
#
# [CRITICAL-2] Per-action continuous parameter vectors (Paper Eq. 3).
#              Original code: ParamNet outputs ONE (speed, heading) vector
#              shared across all 1 369 joint discrete actions.
#              Fixed: ParamNet outputs K=37 separate (speed, heading) vectors,
#              one per primary discrete action (decoy slot d1), so the agent
#              can learn action-conditional kinematics ("when deploying decoy 5
#              turn hard; when deploying decoy 12 accelerate").
#              QNet now outputs num_secondary=37 Q-values (over d2) given the
#              state and the continuous params for a specific d1.
#
# [MODERATE-1] Ghost gradients in q_net during actor update.
#              Original: q_net accumulated unreported .grad tensors from the
#              actor loss because it was not detached. Fixed by temporarily
#              setting q_net parameters to requires_grad=False during the actor
#              backward pass.
#
# [MODERATE-2] Hyperparameters corrected to match Table I of the paper:
#              batch_size 64 -> 256
#              gamma      0.990 -> 0.995
#              lr_actor   1e-4  -> 2.77e-4
#              lr_critic  1e-3  -> 3.96e-4
#              tau        0.005 -> 0.005032
#
# [MINOR-1]   Added reset_noise() convenience method. Training loop should
#             call agent.reset_noise() at the start of every episode so that
#             the OU process does not carry state across episode boundaries.
#             (Training_DS.py must add this call; see comment below.)
# =============================================================================
...
class PDQNAgent:
    ...
    def __init__(self,
                 ...
                 lr_actor:  float  = 2.77e-4,
                 lr_critic: float  = 3.96e-4,
