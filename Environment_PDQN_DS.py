...
# =============================================================================
# FIXES APPLIED vs original Environment_PDQN_DS.py
# =============================================================================
# [CRITICAL-A] reset() ignored the curriculum's spawn-distance range. Line 82
#              was hardcoded to np.random.uniform(2000.0, 2500.0) regardless
#              of self.min_spawn_dist / self.max_spawn_dist set by training.
#              -> now uses self.min_spawn_dist and self.max_spawn_dist
#                 (also exposed as constructor args).
#
# [CRITICAL-B] step() read pitch from auv_state[9] but index 9 is angular
#              velocity p (theta is at index 7 per _get_observation layout).
#              -> fixed index. theta still stays 0 for the planar AUV model
#                 the action space allows (only speed + heading commands),
#                 but the index is no longer wrong.
#
# [CRITICAL-C] prev_min_distance was never updated mid-episode (the assignment
#              was commented out), so the dense distance shaping signal froze
#              at the episode-start value.
#              -> assignment restored at the end of step().
#
# [CRITICAL-D] R_dist formula was current/prev**2 (dimensionally broken).
#              -> Replaced with the per-paper interpretation:
#                 R_dist_t = current_min_distance / initial_min_distance
#                 (Paper Eq. 11 — "normalized minimum gain in distance from
#                  episode start to the present time step")
#                 R_survive uses current min distance, not the frozen value.
#
# [MINOR-E]    Added "survived" to info dict (= not hit_auv). The original
#              "won" flag was True only when ALL torpedoes were destroyed by
#              decoys, so episodes truncated at max_steps without an AUV hit
#              were counted as failures. Survival rate must use "survived".
# =============================================================================


# Reward weighting hyperparameters (Paper Table I)
EPSILON1 = 1.1560594530854869         # weight for R_dist
EPSILON2 = 1.3988588210079729         # weight for R_survive
SCALING_FACTOR = 17.574657210036534   # hit penalty scaling
DECOY_REWARD = 323.54720558089986     # bonus per torpedo destroyed by decoy
...
class AuvEvasionEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        for i in range(self.n):
            dist      = np.random.uniform(self.min_spawn_dist, self.max_spawn_dist)
    ...
    def _compute_reward(self, decoy_error, hit_auv, all_destroyed,
        ...
        [CRITICAL-D] R_dist now correctly computes the per-step normalised
        distance gain rather than the broken current/prev**2 ratio.
        """
        eps1 = EPSILON1
        eps2 = EPSILON2
        scaling_factor = SCALING_FACTOR

        # (1) Distance reward: ratio of current min distance to initial min distance
        #     (Eq. 11 -- "normalized minimum gain in distance from start to t").
        #     >=1 means AUV has gained or maintained distance; <1 means torpedoes
        #     have closed in.
        denom_init = self.initial_min_distance + 1e-6
        R_dist     = self.current_min_distance / denom_init

        # (2) Survival reward: per-step bonus for staying alive (Eq. 12).
        #     Larger when AUV is close to a torpedo (high-danger survival is more
        #     valuable than safe-distance survival).
        denom_curr = self.current_min_distance + 1e-6
        R_survive  = (self.max_steps * self.torpedo_speed) / denom_curr

        # (3) Decoy bonus: terminal-style bonus per torpedo destroyed by decoy.
        R_decoy_bonus = DECOY_REWARD * torpedoes_destroyed

        # (4) Hit penalty: scales the previous step's unpenalised reward and
        #     negates it (Eq. 13).
        R_hit_penalty = 0.0
        if hit_auv:
        ...
        # Memoise the unpenalised step reward to use as R_{t-1} on a future hit
