...
# =============================================================================
# FIXES APPLIED vs original Training_DS.py
# =============================================================================
# [MINOR-1]  Added  agent.reset_noise()  at the start of every episode so the
#            OU process does not carry state across episode boundaries.
#
# [MINOR-2]  Survival counting now uses info["survived"] (= not hit_auv) which
#            correctly counts truncated-but-not-hit episodes as survivals.
#            Previously the code used info["won"] which only fires when ALL
#            torpedoes are destroyed by decoys.
#
# [MOD-1]    DECAY_EPISODES_PER_LEVEL  20000 -> 500  (Paper Table I).
#
# [MOD-2]    Curriculum simplified to match Paper Algorithm 1: incremental
#            torpedo count (1 -> 2 -> 3) at the full distance range used in
#            the paper's evaluation (1500..2400 m). The two-step warm-up at
#            1 torpedo with restricted range was removed — the paper does not
#            describe it.
#
# [MOD-3]    Final checkpoint now saves all four inference-time networks
#            (actor_encoder, critic_encoder, param_net, q_net) plus the agent
#            config in a single file so inference scripts can rebuild the
#            agent from one load() call.
#
# [MOD-4]    Added a hard episode budget so the training loop cannot run
#            forever if a stage never reaches its target survival rate.
# =============================================================================


# ------------------------- Exploration schedule -------------------------------
NOISE_START   = 1.0     # Paper Table I
NOISE_END     = 0.05
EPSILON_START = 1.0
EPSILON_END   = 0.10
DECAY_EPISODES_PER_LEVEL = 500          # [MOD-1] was 20 000 in the original
...
# [MOD-2] Incremental-torpedoes curriculum at the paper's full evaluation
# distance range. After convergence at numtor=3 the DeepSets architecture
# generalises zero-shot to higher torpedo counts at inference.
CURRICULUM_STAGES = [
...
def train_auv():
    ...
    # Agent uses the paper's Table I hyperparameters by default
    agent = PDQNAgent(
    ...
    while training_active:
        ...
        # --- Episode setup ---------------------------------------------------
        state, _ = env.reset()
        agent.reset_noise()           # [MINOR-1] OU noise reset between episodes
        episode_reward = 0.0
        done           = False
        ...
        # --- Episode bookkeeping --------------------------------------------
        # [MINOR-2] Survival = AUV not hit (covers both "all torps destroyed"
        # and "ran out the clock without being hit")
